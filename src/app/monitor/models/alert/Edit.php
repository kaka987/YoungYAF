...
class Model_Alert_Edit
{
    ...
    public function getUserNameByGid($gid) {

        $sql = "select u.aliasname,u.fullname from {$this->monitorRelationUGTable} as ug,{$this->monitorUserTable} as u
    ...
    public function insertRLA($logid,$workers,$actions) {
        ...
        if (!$this->dao->queryRow($sql, true))
            return $this->dao->insert($this->monitorRelationLATable,array('logid'=>$logid,'workerid'=>$workers,'actionid'=>$actions));
        return FALSE;
    }

    public function deleteRLA($rlaid=0) {
